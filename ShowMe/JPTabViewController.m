...
- (void)viewDidLoad
{
    ...
    _indicatorView.backgroundColor= [UIColor colorWithRed:255.0/255.0 green:206.0/255.0 blue:0.0/255.0 alpha:1.0];
...
- (void)loadUI
{
    ...
    float tabWidth = (self.view.frame.size.width / [_controllers count]) + 3.0;
    ...
    for (int i=0; i < [_controllers count]; i++)
    {
        ...
        [tab setBackgroundColor:[UIColor whiteColor]];
         if([controller.title isEqualToString:@"list"]){
            [tab setImage:[UIImage imageNamed:@"user_follow"] forState:UIControlStateNormal];
            
        }else if([controller.title isEqualToString:@"bonus"]){
            [tab setImage:[UIImage imageNamed:@"user_bonus"] forState:UIControlStateNormal];
        }else{
            [tab setImage:[UIImage imageNamed:@"user_event"] forState:UIControlStateNormal];
        }
        ...
        if( i>0 )
        {
            UIView *sep = [[UIView alloc] initWithFrame:CGRectMake(i * tabWidth,
                                                                   ...
                                                                   1,
                                                                   20)];
            [sep setBackgroundColor:[UIColor colorWithRed:255.0/255.0 green:206.0/255.0 blue:0.0/255.0 alpha:1.0]];
    ...
    selectedTab = 0;
    _indicatorView.frame = CGRectMake(0.0, _menuHeight + _statusHeight - 3.0, tabWidth, 3.0);
