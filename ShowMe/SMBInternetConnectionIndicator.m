...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        gradient.colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithRed:0.8 green:0 blue:0 alpha:0.8] CGColor], (id)[[UIColor colorWithRed:0.8 green:0 blue:0 alpha:0.8] CGColor], nil];
        [indicatorView.layer insertSublayer:gradient atIndex:0];
        /** GRADIENT RED BACKGROUND COLOR **/
        
        UIImage *image = [UIImage imageNamed:@"NetworkIcon1"];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        [imageView sizeToFit];
        
        indicatorLabel                                  = [[UILabel alloc] init];
        indicatorLabel.font                             = [UIFont fontWithName:@"Roboto-Light" size:14];
        indicatorLabel.textColor                        = [UIColor whiteColor];
        indicatorLabel.shadowColor                      = [UIColor blackColor];
        indicatorLabel.shadowOffset                     = CGSizeMake(0, 0);
        indicatorLabel.text                             = @"Интернэт шаардлагатай";
        ...
        indicatorLabel.center = CGPointMake(imageView.frame.origin.x + imageView.frame.size.width*1.25 + indicatorLabel.frame.size.width/2, imageView.center.y + 3);
        
        [indicatorView addSubview:imageLabelView];
        
        //Change the host name here to change the server you want to monitor.
        NSString *remoteHostName = @"google.com";
...
- (void)reachabilityChanged:(NSNotification *)note {
    ...
    if(netStatus) {
       // tIndicFrame.origin.y -=indicatorView.frame.size.height;
        ...
        tIndicFrame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 30);
