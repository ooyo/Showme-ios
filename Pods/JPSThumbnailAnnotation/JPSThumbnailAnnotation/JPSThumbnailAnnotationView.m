...
static CGFloat const kJPSThumbnailAnnotationViewStandardHeight    = 90.0f;
...
@property (nonatomic, strong) UIColor *pinBGColor;
...
- (void)setupImageView {
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(10.5f, 10.5f, 54.0f, 54.0f)];
    _imageView.layer.cornerRadius = _imageView.layer.frame.size.width / 2;
...
- (void)setLayerProperties {
    ...
    _bgLayer.path = path;
    CFRelease(path);
    _bgLayer.fillColor = _pinBGColor.CGColor;// [UIColor whiteColor].CGColor;
    
    _bgLayer.shadowColor = [UIColor blackColor].CGColor;
    _bgLayer.shadowOffset = CGSizeMake(0.0f, 0.0f);
    _bgLayer.shadowRadius = 0.0f;
    _bgLayer.shadowOpacity = 0.0f;
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    self.bgLayer.fillColor = thumbnail.pinBGColor.CGColor;
}
...
- (CGPathRef)newBubbleWithRect:(CGRect)rect {
    ...
    CGPathAddLineToPoint(path, NULL, parentX - 14.0f, rect.origin.y-3  + rect.size.height);
